...
internal class ReadWriteMutexImpl : ReadWriteMutex {
    ...
    internal fun writeUnlock(policy: WriteUnlockPolicy) {
        while (true) {
            val state = STATE.value
            val resumeWriter = (state.ww > 0) && (policy == PRIORITIZE_WRITERS || policy == RANDOM && Random.nextBoolean())
            if (resumeWriter) {
                if (STATE.compareAndSet(state, constructState(0, true, state.ww - 1, false))) {
                    sqsWriters.resume(Unit)
                    return
                ...
                if (STATE.compareAndSet(state, constructState(1, false, state.ww, true))) {
                    completeWriteUnlock()
                    return
                }
            }
