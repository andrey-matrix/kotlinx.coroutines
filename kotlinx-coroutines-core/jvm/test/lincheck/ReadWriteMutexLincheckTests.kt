...
internal abstract class ReadWriteMutexLincheckTestBase(
    private val writeUnlockPolicy: WriteUnlockPolicy,
    private val seqSpec: KClass<out ReadWriteMutexLincheckTestSequential>
    ...
    fun writeUnlock(@Param(gen = ThreadIdGen::class) threadId: Int): Boolean {
        ...
        m.writeUnlock(writeUnlockPolicy)
...
internal class ReadWriteMutexPrioritizeWritersLincheckTestBase : ReadWriteMutexLincheckTestBase(PRIORITIZE_WRITERS, ReadWriteMutexPrioritizeWritersLincheckTestSequential::class)
internal class ReadWriteMutexPrioritizeReadersLincheckTestBase : ReadWriteMutexLincheckTestBase(PRIORITIZE_READERS, ReadWriteMutexPrioritizeReadersLincheckTestSequential::class)

internal abstract class ReadWriteMutexLincheckTestSequential(writeUnlockPolicy: WriteUnlockPolicy) : VerifierState() {
    private val m = ReadWriteMutexSequential(writeUnlockPolicy)
    private val readLockAcquired = IntArray(6)
    private val writeLockAcquired = BooleanArray(6)
    ...
    override fun extractState() = "mutex=${m.state},rlaPerThread=${readLockAcquired.contentToString()},wlaPerThread=${writeLockAcquired.contentToString()}"
}
internal class ReadWriteMutexPrioritizeWritersLincheckTestSequential : ReadWriteMutexLincheckTestSequential(PRIORITIZE_WRITERS)
internal class ReadWriteMutexPrioritizeReadersLincheckTestSequential : ReadWriteMutexLincheckTestSequential(PRIORITIZE_READERS)
...
internal class ReadWriteMutexSequential(
    ...
    fun writeUnlock() {
        if (ww.isNotEmpty() && writeUnlockPolicy == PRIORITIZE_WRITERS) {
            val w = ww.removeAt(0)
            w.resume(Unit) { writeUnlock() }
        } else {
            wla = false
